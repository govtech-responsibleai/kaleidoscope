...
@pytest.mark.unit
def test_ensure_system_judges_scopes_to_target_system_rubrics(test_db, sample_target, other_target):
    accuracy_name = get_fixed_template("accuracy")["name"]
    ensure_system_rubrics(test_db, sample_target.id)
    ...
    assert len(sample_fixed_judges) == 3
    assert len(sample_preset_judges) == 3
